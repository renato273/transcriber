...
import { Users, Shield, RefreshCw, UserCog } from 'lucide-react';
  ...
  return (
    <div class="max-w-4xl w-full mx-auto p-3 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 min-h-[calc(100dvh-3.5rem)] sm:min-h-[calc(100vh-4rem)] flex flex-col justify-start">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-gray-400 mt-1.5">
              Asigna rol ADMIN o USER. Solo administradores acceden a API keys y esta sección.
            </p>
          </div>
          <button
            type="button"
        ...
        {users.length === 0 ? (
          ...
          users.map((user) => {
            ...
            return (
              <div
                key={user.id}
                class="p-4 sm:p-5 bg-[#151D30]/40 border border-[#1F293D] rounded-2xl flex flex-col sm:flex-row sm:items-center gap-4"
              >
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  <select
                    ...
                    title={isLastAdmin ? 'No se puede degradar al único admin' : 'Cambiar rol'}
                  ...
                  {busy && <RefreshCw class="w-4 h-4 animate-spin text-primary" />}
                </div>
              </div>
            );
          })
      ...
      <p class="text-[11px] text-gray-500 flex items-start gap-2">
        ...
        Debe existir al menos un ADMIN. El cambio de rol aplica en la siguiente petición del usuario.
      </p>
    </div>
  );
}
